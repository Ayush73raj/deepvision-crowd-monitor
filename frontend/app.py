...
def autoplay_audio(file_path, loop=False):
    ...
    st.markdown(
        f"""
        <audio id="alertAudio" autoplay {loop_attr}>
...
def show_blinking_banner(text, color="red"):
    st.markdown(
        f"""
        <style>
        @keyframes blink {{
                ...
                if count is not None:
                    ...
                    if enable_alerts:
                        ...
                        if count > threshold_critical:
                            show_blinking_banner("🚨🚨 CRITICAL CROWD OVERLOAD 🚨🚨", "red")
                            st.error(f"### 🚨 CRITICAL ALERT")
                            st.error(action)

                            if enable_sound and sound_path.exists():
                                autoplay_audio(sound_path, loop=True)

                        elif count > threshold_high:
                            show_blinking_banner("⚠️ HIGH RISK CROWD DENSITY ⚠️", "orange")
